...
SOCKET_PATH = os.environ.get(
    ...
    os.path.expanduser("~/.claude-island/ci.sock"),
)
...
def _install_claude_hook(socket_path):
    ...
    dst_script = os.path.join(hooks_dir, "claude-island-state.py")

    # Persist socket path for other helpers (e.g. OpenCode plugin).
    state_dir = os.path.join(home, ".claude-island")
    ...
    cmd = f'CLAUDE_ISLAND_SOCKET_PATH="{quoted_sock}" python3 ~/.claude/hooks/claude-island-state.py'
...
def _install_opencode_plugin(socket_path):
    ...
    dst_plugin = os.path.join(plugins_dir, "claude-island.js")
    _ensure_dir(plugins_dir)

    resource_dir = os.path.dirname(os.path.abspath(__file__))
    src_plugin = os.path.join(resource_dir, "claude-island-opencode.js")
    ...
    state_dir = os.path.expanduser("~/.claude-island")
    _ensure_dir(state_dir)
    try:
        with open(os.path.join(state_dir, "socket-path"), "w", encoding="utf-8") as f:
            f.write(str(socket_path).strip() + "\n")
...
def uninstall_all():
    ...
    hooks_dir = os.path.join(claude_dir, "hooks")
    settings_path = os.path.join(claude_dir, "settings.json")
    script_path = os.path.join(hooks_dir, "claude-island-state.py")
        ...
        for event in list(hooks.keys()):
            ...
            for entry in entries:
                if not isinstance(entry, dict):
                    ...
                    continue
                hs = entry.get("hooks")
                if isinstance(hs, list) and any("claude-island-state.py" in (h.get("command") or "") for h in hs if isinstance(h, dict)):
                    continue
                new_entries.append(entry)
            if new_entries:
    ...
    opencode_plugin = os.path.join(home, ".config", "opencode", "plugins", "claude-island.js")
    try:
